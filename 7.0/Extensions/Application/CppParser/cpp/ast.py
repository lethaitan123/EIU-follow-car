...
class Function(_GenericDeclaration):
         ...
         for par in self.parameters:
              if self.parameters.index(par) == len(self.parameters)-1:
                  ...
                  if par.name is None:
                      ...
                      pars='%s %s, '%(par.type, par.name)
              parstring+=pars
         if(parstring.endswith(",")): parstring = parstring.rstrip(',')
         return parstring
    ...
    def __str__(self):
        ...
        if(rType is None):
            signature='%s(%s);' % (self.name,  param);
        else:
            signature='%s %s(%s);' % (self.return_type.name, self.name,  param);
        return signature
